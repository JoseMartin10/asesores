...
function calcular() {
    ...
    var agregarSubsidio = parseInt(document.getElementById("agregarSubsidio").value);
    ...
    } else if (agregarSubsidio === 2) {
        if (ingresos < SMMLV * 2 && puntajeSisben == 1) {
            subsidio = 0;
            subsidioCompensacion = 39000000;
            valorCuotaInicial = (valorVivienda * 0.1) - separacion;
            credito = valorVivienda - subsidioCompensacion - valorCuotaInicial;
        } else {
            switch (puntajeSisben) {
                case 1:
                    subsidio = 0;
                ...
                case 2:
                    subsidio = 0;
                    subsidioCompensacion = 26000000;
                    valorCuotaInicial = (valorVivienda * 0.3) - subsidioCompensacion - separacion;
                    credito = valorVivienda - subsidioCompensacion - valorCuotaInicial;
                    break;
                case 3:
                    subsidio = 0;
        ...
        if (ingresos < SMMLV * 2 && puntajeSisben == 1) {
            ...
            switch (puntajeSisben) {
                ...
                case 2:
                    subsidio = 26000000;
                    subsidioCompensacion = 26000000;
                    valorCuotaInicial = (valorVivienda * 0.3) - subsidio - separacion - subsidioCompensacion;
                    credito = valorVivienda - subsidio - valorCuotaInicial - subsidioCompensacion;
                    break;
                case 3:
                    subsidio = 0;
    ...
    } else if (agregarSubsidio === 0) {
        subsidio = 0;
        subsidioCompensacion = 0;
        if (ingresos < SMMLV * 2 && puntajeSisben == 1) {
            valorCuotaInicial = (valorVivienda * 0.1) - separacion;
        } else {
            valorCuotaInicial = puntajeSisben == 1 ? (valorVivienda * 0.1) - separacion : (valorVivienda * 0.3) - separacion;
        }
        credito = valorVivienda - valorCuotaInicial;
    }
    ...
    var elementoCesantia = document.getElementById("selectCesantia");
    if (elementoCesantia) {
        numeroCuotaCesantia = elementoCesantia.value ?? 0;
    }

    var elementoPrima = document.getElementById("selectPrima");
    if (elementoPrima) {
        numeroCuotaPrima = elementoPrima.value ?? 0;
    }
    ...
    var cuotaMaxima = parseFloat(document.getElementById("cuotaMaxima").value);
    var ultimaCuota = valorCuotaInicial - separacion - cuotaMaxima;

    if (cuotaMaxima > 0) {
        cuotaMensualInicial = cuotaMaxima
        // console.log("valor de la ultima cuota:", ultimaCuota)
        // console.log("valor de la cuota maxima:", cuotaMaxima)
    };
    ...
    ${mesesRestantes > 0 ? `<p><strong> cuotas mensuales de: </strong><br>${formatNumberWithSpaces(cuotaMensualInicial)}</p>` : ''}
    ...
    ${ultimaCuota > 0 ? `<p><strong>Pagarás una última cuota por el valor de: </strong><br>${formatNumberWithSpaces(ultimaCuota)}</p>` : ''}
...
function generarImagen() {
    // Usar html2canvas para capturar el contenido del div completo, sin cortar
